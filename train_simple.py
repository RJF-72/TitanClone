"""
Simple Single-GPU Training Script for so-vits-svc
...
def main():
    ...
    try:
        ...
        if latest_g and latest_d:
            ...
            global global_step
            global_step = int(latest_d[latest_d.rfind("_")+1:latest_d.rfind(".")]) + 1
            print(f"Resumed from epoch {epoch_str}, step {global_step}")
    except Exception as e:
        print(f"load old checkpoint failed: {e}")
        epoch_str = 1
        global_step = 0
    ...
    print(f"\nStarting training from epoch {epoch_str}...")
    print(f"Training samples: {len(train_dataset)}")
    print(f"Batch size: {hps.train.batch_size}")
    print(f"Steps per epoch: {len(train_loader)}")
    print(f"Checkpoint every: {hps.train.eval_interval} steps\n")
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        ...
        train_and_evaluate(epoch, hps, [net_g, net_d], [optim_g, optim_d], 
                          [scheduler_g, scheduler_d], scaler, 
                          [train_loader, eval_loader], logger, 
                          [writer, writer_eval], device, half_type)
        
        scheduler_g.step()
        scheduler_d.step()


def train_and_evaluate(epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers, device, half_type):
    net_g, net_d = nets
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        if global_step % hps.train.log_interval == 0:
            ...
            reference_loss = sum([x.item() for x in losses])
            logger.info(f'Train Epoch: {epoch} [{100. * batch_idx / len(train_loader):.0f}%]')
            logger.info(f"Losses: {[x.item() for x in losses]}, step: {global_step}, lr: {lr}, total: {reference_loss}")
            print(f"Epoch {epoch} Step {global_step}: loss={reference_loss:.2f}")
            ...
            scalar_dict.update({
                ...
                "loss/g/kl": loss_kl, "loss/g/lf0": loss_lf0
            })
        ...
        if global_step % hps.train.eval_interval == 0:
            ...
            print(f"CHECKPOINT SAVED: G_{global_step}.pth")
